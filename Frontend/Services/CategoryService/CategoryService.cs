    ...
    public class CategoryService : ICategoryService
    {
        public async Task CreateCategory(Category category)
        {
            using(var client = new HttpClient())
            {
                ...
                {
                    RequestUri = new Uri("https://backendcategory.azurewebsites.net/api/category")
                };
        ...
        public async Task DeleteCategory(Guid? id)
        {
            using(var client = new HttpClient())
            {
                var request = new HttpRequestMessage
                {
                    RequestUri = new Uri($"https://backendcategory.azurewebsites.net/api/category/{id}")
                };
        ...
        public async Task UpdateCategory(Category category)
        {
            using (var client = new HttpClient())
            {
                var request = new HttpRequestMessage
                {
                    RequestUri = new Uri($"https://backendcategory.azurewebsites.net/api/category/{category.Id}")
                };
                var json = JsonConvert.SerializeObject(category);
